class Expense < ApplicationRecord
  ...
  def set_activity(user_name, old_description= '', old_date= '', old_amount = '')
    ...
    if !old_description.blank? && old_description != self.description
      self.activities.create(name: "Description changed #{old_description} to #{self.description}  by #{user_name} at #{DateTime.now.utc}")
    end
    if !old_date.blank? && old_date != self.date
      self.activities.create(name: "Date changed #{old_date} to #{self.date} by #{user_name} at #{DateTime.now.utc}")
    end
  end
end
